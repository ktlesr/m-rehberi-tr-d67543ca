...
export function AIEvidencePanel({ evidence, issues, missingTags, onClose }: AIEvidencePanelProps) {
  ...
  // Position and size refs for smooth updates (no re-renders during drag/resize)
  const positionRef = useRef({ x: window.innerWidth - 420, y: 80 });
  const sizeRef = useRef({ width: 384, height: 500 });
  const startRef = useRef({ x: 0, y: 0, initialX: 0, initialY: 0, initialW: 0, initialH: 0 });
  
  // RAF id for cleanup
  const rafRef = useRef<number | null>(null);

  // Apply position/size to DOM directly via rAF
  ...
  const handlePointerMove = useCallback((e: PointerEvent) => {
    ...
    if (modeRef.current === 'drag') {
      const newX = Math.max(0, Math.min(window.innerWidth - sizeRef.current.width, startRef.current.initialX + deltaX));
      const newY = Math.max(0, Math.min(window.innerHeight - 100, startRef.current.initialY + deltaY));
      positionRef.current = { x: newX, y: newY };
    } else if (modeRef.current === 'resize') {
      const newWidth = Math.max(320, Math.min(800, startRef.current.initialW + deltaX));
      const newHeight = Math.max(300, Math.min(window.innerHeight - 100, startRef.current.initialH + deltaY));
      sizeRef.current = { width: newWidth, height: newHeight };
    }

    // Use rAF for smooth updates
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    rafRef.current = requestAnimationFrame(updatePanelStyle);
  }, [updatePanelStyle]);
  ...
  const handleDragStart = useCallback((e: React.PointerEvent) => {
    ...
    startRef.current = {
      ...
      initialW: sizeRef.current.width,
      initialH: sizeRef.current.height,
    };
  ...
  }, [updatePanelStyle, handlePointerMove, handlePointerUp]);
  ...
  return (
    ...
    >
      {/* Draggable Header */}
      <div 
        className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-purple-50 to-blue-50 rounded-t-lg cursor-move select-none flex-shrink-0 touch-none"
      ...
      {/* Resize Handle - Larger area for easier grabbing */}
      <div 
        className="absolute bottom-0 right-0 w-5 h-5 cursor-se-resize group touch-none"
